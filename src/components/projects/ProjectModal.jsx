...
import { X, FolderKanban, User, Trash2, Globe, Lock, LayoutTemplate } from 'lucide-react'

export default function ProjectModal({ project, onClose, onSave, onDelete, onSaveAsTemplate }) {
  ...
  async function handleSave() {
    ...
    const payload = { name: name.trim(), description, type, visibility, status: project?.status || 'active' }
    const result = await onSave(payload)
    if (result?.error) { setError(result.error.message || 'Erro ao salvar'); setLoading(false); return }

    // salvar como template se toggle ativado
    if (saveAsTemplate && onSaveAsTemplate) {
      await onSaveAsTemplate({ name: name.trim(), description, type, visibility })
    }
    onClose()
    setLoading(false)
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'var(--overlay)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-red)', borderRadius: 'var(--radius-md)', width: '100%', maxWidth: 460, padding: 28 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[
              ...
              <button key={val} onClick={() => setType(val)} style={{
                ...
                background: type === val ? 'var(--red-dim)' : 'var(--surface)', transition: 'all var(--fast)',
              }}>
                ...
                <span style={{ fontFamily: 'var(--ff-mono)', fontSize: 9, color: 'var(--text-dim)' }}>{sub}</span>
              </button>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[
              ...
              <button key={val} onClick={() => setVisibility(val)} style={{
                ...
                background: visibility === val ? 'var(--red-dim)' : 'var(--surface)', transition: 'all var(--fast)',
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {/* ── toggle: salvar como template ── */}
        <button
          onClick={() => setSaveAsTemplate(v => !v)}
          style={{
            width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            padding: '10px 12px', borderRadius: 'var(--radius-md)', marginBottom: 20,
            border: saveAsTemplate ? '1px solid var(--border-red)' : '1px solid var(--border)',
            background: saveAsTemplate ? 'var(--red-dim)' : 'var(--surface)',
            cursor: 'pointer', transition: 'all var(--fast)',
          }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <LayoutTemplate size={13} style={{ color: saveAsTemplate ? 'var(--red)' : 'var(--text-muted)' }} />
            <div style={{ textAlign: 'left' }}>
              <div style={{ fontFamily: 'var(--ff-mono)', fontSize: 11, letterSpacing: '0.08em', color: saveAsTemplate ? 'var(--text)' : 'var(--text-muted)', textTransform: 'uppercase' }}>
                salvar como template
              </div>
              <div style={{ fontFamily: 'var(--ff-mono)', fontSize: 9, color: 'var(--text-dim)', marginTop: 2 }}>
                disponível para reusar em outros projetos
              </div>
            </div>
          </div>
          {/* switch visual */}
          <div style={{ width: 32, height: 18, borderRadius: 9, background: saveAsTemplate ? 'var(--red)' : 'var(--border)', position: 'relative', transition: 'background var(--fast)', flexShrink: 0 }}>
            <div style={{ width: 14, height: 14, borderRadius: '50%', background: '#fff', position: 'absolute', top: 2, left: saveAsTemplate ? 16 : 2, transition: 'left var(--fast)' }} />
          </div>
        </button>

        {error && (
          <div style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, color: 'var(--red)', padding: '7px 10px', background: 'var(--red-dim)', border: '1px solid var(--border-red)', borderRadius: 'var(--radius)', marginBottom: 14 }}>
            {error}
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between', alignItems: 'center' }}>
          {editing && onDelete ? (
            <button onClick={handleDelete} disabled={deleting} style={{
              ...
              padding: '7px 12px', borderRadius: 'var(--radius)', cursor: 'pointer', transition: 'all var(--fast)',
            }}>
              <Trash2 size={12} />
