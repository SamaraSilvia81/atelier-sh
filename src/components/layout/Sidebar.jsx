...
function SidebarContent({
  ...
  orgSaved,
  isDark, toggleTheme,
  navBtn, isActive, navTo,
  selectOrg, selectProject,
  setShowOrgModal, setEditingOrg,
  setShowProjectModal, setEditingProject,
  setShowMembers,
  setMobileOpen,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: c ? '16px 0' : '18px 18px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: c ? 'center' : 'space-between', minHeight: 62 }}>
        {c ? (
          ...
          <>
            ...
            {isMobile && <button onClick={() => setMobileOpen(false)} style={{ color: 'var(--text-muted)', padding: 4, background: 'none', border: 'none', cursor: 'pointer' }}><X size={18} /></button>}
          </>
        )}
      </div>
      ...
      <nav style={{ flex: 1, padding: c ? '10px 6px' : '10px 8px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        {isAdmin && (
          <button onClick={() => setShowMembers(true)} title={c ? 'membros' : ''}
        ...
        <a href={EXTENSION_URL} target="_blank" rel="noreferrer" title={c ? 'extensão chrome' : ''}
          style={{ ...navBtn(false), textDecoration: 'none', marginTop: 8, borderStyle: 'dashed', color: 'var(--red)', borderColor: 'var(--border-red)', opacity: 0.7 }}>
      ...
      <div style={{ padding: c ? '8px 6px' : '8px 8px', borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <button onClick={toggleTheme} title={isDark ? 'light mode' : 'dark mode'}
          style={{ display: 'flex', alignItems: 'center', justifyContent: c ? 'center' : 'space-between', padding: c ? '9px 0' : '7px 10px', borderRadius: 'var(--radius)', fontFamily: 'var(--ff-mono)', fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text-muted)', border: '1px solid var(--border)', background: 'var(--surface)', cursor: 'pointer' }}>
        ...
        <button onClick={signOut} title="sair"
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: c ? '9px 0' : '7px 10px', borderRadius: 'var(--radius)', fontFamily: 'var(--ff-mono)', fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text-dim)', border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer' }}>
...
export default function Sidebar({ currentOrgId, setCurrentOrgId, currentProjectId, setCurrentProjectId, collapsed, setCollapsed }) {
  ...
  function selectProject(id) { setCurrentProjectId(id); if (location.pathname !== '/') navigate('/'); setMobileOpen(false) }
  function selectOrg(id) {
    setCurrentOrgId(id); setCurrentProjectId(null); setOrgSaved(true)
  ...
  return (
    <>
      <aside className="mobile-hidden" style={{ width: collapsed ? 'var(--sidebar-col)' : 'var(--sidebar-w)', minHeight: '100vh', background: 'var(--bg-alt)', borderRight: '1px solid var(--border)', position: 'fixed', left: 0, top: 0, bottom: 0, zIndex: 100, transition: 'width var(--mid) var(--ease)', overflow: 'visible' }}>
        <SidebarContent {...contentProps} col={col} isMobile={false} />
      ...
      <button onClick={() => setMobileOpen(true)} style={{ display: 'none', position: 'fixed', top: 14, left: 16, zIndex: 30, width: 36, height: 36, borderRadius: 'var(--radius-md)', background: 'var(--bg-card)', border: '1px solid var(--border-red)', color: 'var(--red)', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }} className="mobile-menu-btn">
        <Menu size={16} />
      </button>
      ...
      {mobileOpen && (
        <>
          ...
          <div style={{ position: 'fixed', left: 0, top: 0, bottom: 0, width: 280, background: 'var(--bg-alt)', borderRight: '1px solid var(--border)', zIndex: 50, overflowY: 'auto' }}>
            <SidebarContent {...contentProps} col={false} isMobile={true} />
          </div>
        </>
      )}
