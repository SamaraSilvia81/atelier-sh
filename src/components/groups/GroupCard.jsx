      ...
      <div className="card" style={{
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 160 }}>
            <span style={{ fontFamily: 'var(--ff-disp)', fontSize: '0.95rem', letterSpacing: '0.04em', color: 'var(--text)', lineHeight: 1.1 }}>{group.name}</span>
          ...
          {group.description && <span style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{group.description}</span>}
          {members.length > 0 && <span style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, color: 'var(--text-dim)', display: 'flex', alignItems: 'center', gap: 3 }}><Users size={9} /> {members.length}</span>}
        </div>
    ...
    <>
      <div className="card">
        ...
        <div style={{ padding: '14px 18px 10px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              <span style={{ fontFamily: 'var(--ff-disp)', fontSize: '1.05rem', letterSpacing: '0.04em', color: 'var(--text)', lineHeight: 1.1 }}>{group.name}</span>
              {isPrivate && <Lock size={10} style={{ color: 'var(--amber)', flexShrink: 0 }} />}
              {group.figma_url && <span title="tem Figma" style={{ color: '#a259ff', fontSize: 10 }}>◈</span>}
              {group.is_template && <span style={{ fontFamily: 'var(--ff-mono)', fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', padding: '2px 6px', borderRadius: 'var(--radius)', background: 'rgba(200,146,42,0.15)', border: '1px solid rgba(200,146,42,0.4)', color: '#c8922a' }}>template</span>}
              {members.length > 0 && <span title={`${members.length} integrante${members.length !== 1 ? 's' : ''}`} style={{ display: 'flex', alignItems: 'center', gap: 2, fontFamily: 'var(--ff-mono)', fontSize: 9, color: 'var(--text-dim)', letterSpacing: '0.1em' }}><Users size={9} /> {members.length}</span>}
            </div>
            {group.description && <div style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: 3 }}>{group.description}</div>}
            {(group.stage || tags.length > 0) && (
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 6 }}>
                {group.stage && <span style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, padding: '2px 7px', border: '1px solid var(--border)', borderRadius: 'var(--radius)', color: 'var(--text-dim)', background: 'var(--surface)' }}>{group.stage}</span>}
                {tags.slice(0, 3).map(tag => <span key={tag} style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, padding: '2px 7px', border: '1px solid var(--border-red)', borderRadius: 'var(--radius)', color: 'var(--text-muted)', background: 'rgba(74,21,21,0.1)' }}>{tag}</span>)}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 3 }}>
              {actionBtn('anotações', <FileText size={11} />, () => onOpenNotes(group))}
              {actionBtn('⬡ ativar extensão', <Monitor size={11} />, () => onOpenReview && onOpenReview(group))}
              {onDuplicate && actionBtn('duplicar', <Copy size={11} />, () => onDuplicate(group.id))}
        ...
        <div style={{ padding: '10px 18px', fontFamily: 'var(--ff-mono)', fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6 }}>
          {loading.gh ? <span style={{ color: 'var(--text-dim)' }}>carregando_</span>
            ...
            : github?.lastCommit ? <>
                ...
                <div style={{ color: 'var(--text-dim)', fontSize: 11, marginTop: 3 }}>{github.lastCommit.author} · {github.lastCommit.sha}</div>
              </>
          ...
          <div style={{ padding: '0 18px 10px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
              {atas.slice(0, 4).map(a => (
                <a key={a.name} href={a.url} target="_blank" rel="noopener" style={{ fontFamily: 'var(--ff-mono)', fontSize: 11, padding: '3px 8px', border: '1px solid var(--border-red)', borderRadius: 'var(--radius)', color: 'var(--text-muted)', background: 'rgba(74,21,21,0.12)', textDecoration: 'none' }} onMouseEnter={e => e.currentTarget.style.color = 'var(--text)'} onMouseLeave={e => e.currentTarget.style.color = 'var(--text-muted)'}>{a.name.replace('.md', '')}</a>
              ))}
        ...
        {members.length > 0 && (
          <div style={{ padding: '0 18px 10px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
              {members.slice(0, 4).map(m => <span key={m.id} style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, padding: '2px 7px', border: '1px solid var(--border)', borderRadius: 'var(--radius)', color: 'var(--text-muted)', background: 'var(--surface)' }}>{m.name}</span>)}
        ...
        <div style={{ padding: '9px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', gap: 12 }}>
            {group.github_repo && <a href={`https://github.com/${group.github_repo}`} target="_blank" rel="noopener" style={{ fontFamily: 'var(--ff-mono)', fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 4, textDecoration: 'none' }} onMouseEnter={e => e.currentTarget.style.color = 'var(--red)'} onMouseLeave={e => e.currentTarget.style.color = 'var(--text-muted)'}><GitBranch size={10} /> {isPrivate ? 'privado' : 'github'}</a>}
